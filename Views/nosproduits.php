...
<body id="top">
  ...
  <main>
    ...
    <article>
      ...
      <section style="display: flex ; flex-direction:column; justify-content:center; align-items:center" class="section product">
        <div class="container">

          <h2 class="h2 section-title"> Nos produits</h2>
          ...
          <?php
             ...
             // Définissez le nombre d'articles à afficher par page
             $produitParPage  = 4;

             // Calculez le nombre total d'articles
             $totalproducts= count($myProducts);

             // Calculez le nombre total de pages
              $totalPages = ceil($totalproducts / $produitParPage);

              // Déterminez la page actuelle (par défaut à 1 si non spécifiée dans l'URL)
              $pageCourante = isset($_GET['page']) ? $_GET['page'] : 1;

              // Calculez l'indice de départ et de fin pour l'affichage des articles sur la page actuelle
              $indiceDebut = ($pageCourante - 1) * $produitParPage;
              $indiceFin = $indiceDebut + $produitParPage;


              for ($i = $indiceDebut; $i < $indiceFin; $i++) {
                if ($i < $totalproducts) {
                    // Affichez les détails de l'article $articles[$i]
                    ...
                    <form  method="POST">
                      ...
                      <div class="product-card" tabindex="0">
                        ...
                        <figure class="card-banner">
                          <img src="./assets/images/'.$myProducts[$i]['myimage'].'" width="312" height="350" loading="lazy"
                          ...
                          <ul class="card-action-list">
                            ...
                            <li class="card-action-item">
                                ...
                                <button type="button" onclick="checkIfClicked('.$myProducts[$i]['id'].')" name="addtocart'.$myProducts[$i]['id'].'" class="card-action-btn" aria-labelledby="card-label-1">
                            ...
                            <li class="card-action-item">
                              <button type="button" onclick="voirdetails('.$myProducts[$i]['id'].')" class="card-action-btn" aria-labelledby="card-label-3">
                        ...
                        <div class="card-content">
                          ...
                          <div style="font-size:14px; font-weight:bold" class="card-cat">'.$produits -> getProductCategory($myProducts[$i]['id_category'])['libelle'].'</div>
    
                          <h3 class="h3 card-title">
                            <a href="#">'. $myProducts[$i]['libelle'].'</a>
                          </h3>
    
                          <data class="card-price" value="'. $myProducts[$i]['prix'].'">'. number_format( $myProducts[$i]['prix'] ).' FCFA</data>
          ...
          <div style="width:40%; display:flex; flex-direction:row; justify-content:space-around; align-items:center ; margin-top:100px">
          <h3> Page à montrer :</h3>
          <?php for ($page = 1; $page <= $totalPages; $page++) {
                echo '<a style="font-size:20px" href="?page=' . $page . '">' . $page . '</a>';
          }
          ?>
          </div>
        </div>
